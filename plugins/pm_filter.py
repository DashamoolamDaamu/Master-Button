...
def _build_filter_buttons(key: str, req: int, offset: int,
    ...
    row1 = [
        Btn(_lbl("🌐 Language", cur_lang),
            callback_data=f"flt_{req}_{key}_L", color=C.GREEN),
        Btn(_lbl("🎬 Quality",  cur_qual),
            callback_data=f"flt_{req}_{key}_Q", color=C.GREEN),
    ]
    row2 = [
        Btn(_lbl("📺 Season",  cur_season),
            callback_data=f"flt_{req}_{key}_S", color=C.GREEN),
        Btn(_lbl("▶ Episode", cur_ep),
            callback_data=f"flt_{req}_{key}_E", color=C.GREEN),
    ]
    return [row1, row2]
...
def _build_pagination_row(req: int, key: str, offset: int,
    ...
    if offset > 0:
        row.append(Btn(
            "◀ Prev", callback_data=f"next_{req}_{key}_{prev_o}", color=C.GREEN))
    row.append(Btn(
        f"📃 {page + 1} / {pages}", callback_data="pages"))
    if next_o < total:
        row.append(Btn(
            "Next ▶", callback_data=f"next_{req}_{key}_{next_o}", color=C.GREEN))
    return row
...
async def _render_filter_page(
    ...
    if total > 0:
        ...
        btn.append([Btn("📃 0 / 0", callback_data="pages")])
...
async def _send_pm_search_redirect(message):
    ...
    buttons = InlineKeyboardMarkup([[
        Btn("👇 Join / Open Group", url=PM_SEARCH_GROUP_LINK, color=C.BLUE)
    ]])
...
async def filter_open(bot, query):
    ...
    for opt in options:
        ...
        picker_rows.append([Btn(
            text, callback_data=f"fpk_{req}_{key}_{ftype}_{opt}", color=C.GREEN)])

    # Add "Clear" option if one is active
    if cur:
        picker_rows.append([Btn(
            f"✖ Clear {label}", callback_data=f"fpk_{req}_{key}_{ftype}_CLEAR", color=C.RED)])
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        keyboard = InlineKeyboardMarkup([
            [Btn(f"{stat}", callback_data=f"{cb}:{group_id}", color=C.BLUE),
             Btn("DELETE", callback_data=f"deletecb:{group_id}", color=C.RED)],
            [Btn("BACK", callback_data="backcb", color=C.BLUE)]
        ])
...
#ALERT FN IN SPELL CHECK FOR LANGAUGES TO KNOW HOW TO TYPE MOVIES esp english spell check goto adv spell check to check donot change the codes      
        ...
        buttons = [[
            Btn('ᴀᴅᴅ ᴍᴇ ᴛᴏ ʏᴏᴜʀ ɢʀᴏᴜᴘs', url=f'http://t.me/{temp.U_NAME}?startgroup=true', color=C.BLUE)
            ],[
            Btn('ʜᴇʟᴘ', callback_data='help', color=C.BLUE),
            Btn('ᴀʙᴏᴜᴛ', callback_data='about', color=C.BLUE)
        ], [
             Btn(f'ᴏᴛᴛ ᴜᴘᴅᴀᴛᴇs​', url='https://t.me/movie_ottupdates'),
             Btn(f'ᴍᴀɪɴ ᴄʜᴀɴɴᴇʟ', url='https://t.me/mn_movies2')
        ],[
            Btn('🎬 Browse Movies by Genre', callback_data='genre_menu', color=C.GREEN)
        ],[
            Btn('ʀᴇᴘᴏ', url='https://github.com/mn-bots/ShobanaFilterBot')
         ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            Btn('◀️ Pʀᴇᴠ', callback_data='help_page_5', color=C.GREEN),
            Btn('1/6', callback_data='pages'),
            Btn('Nᴇxᴛ ▶️', callback_data='help_page_1', color=C.GREEN)
        ], [
            Btn('Mᴀɴᴜᴀʟ Fɪʟᴛᴇʀ', callback_data='manuelfilter', color=C.BLUE),
            Btn('Aᴜᴛᴏ Fɪʟᴛᴇʀ', callback_data='autofilter', color=C.BLUE)
        ], [
            Btn('Cᴏɴɴᴇᴄᴛɪᴏɴ', callback_data='coct', color=C.BLUE),
            Btn('Exᴛʀᴀ Tʜɪɴɢs', callback_data='extra', color=C.BLUE)
        ], [
            Btn('Hᴏᴍᴇ', callback_data='start', color=C.BLUE),
            Btn('ʀᴇᴘᴏ', url='https://github.com/mn-bots/ShobanaFilterBot')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            Btn('◀️ Pʀᴇᴠ', callback_data=f'help_page_{prev_page}', color=C.GREEN),
            Btn(f'{page + 1}/{total_pages}', callback_data='pages'),
            Btn('Nᴇxᴛ ▶️', callback_data=f'help_page_{next_page}', color=C.GREEN)
        ], [
            Btn('Mᴀɴᴜᴀʟ Fɪʟᴛᴇʀ', callback_data='manuelfilter', color=C.BLUE),
            Btn('Aᴜᴛᴏ Fɪʟᴛᴇʀ', callback_data='autofilter', color=C.BLUE)
        ], [
        ...
        buttons = [[
            Btn('ʙᴀᴄᴋ', callback_data='start', color=C.BLUE),
            Btn('ʀᴇᴘᴏ', url='https://github.com/mn-bots/ShobanaFilterBot')
            ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            Btn('ʙᴀᴄᴋ', callback_data='about', color=C.BLUE),
            Btn('ʀᴇᴘᴏ', url='https://github.com/mn-bots/ShobanaFilterBot')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            Btn('ʙᴀᴄᴋ', callback_data='help', color=C.BLUE),
            Btn('ʀᴇᴘᴏ', url='https://github.com/mn-bots/ShobanaFilterBot'),
            Btn('ʙᴜᴛᴛᴏɴ', callback_data='button', color=C.BLUE)
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            Btn('ʙᴀᴄᴋ', callback_data='help', color=C.BLUE),
            Btn('ᴀᴅᴍɪɴ', callback_data='admin', color=C.BLUE),
            Btn('ʀᴇᴘᴏ', url='https://github.com/mn-bots/ShobanaFilterBot')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            Btn('ʙᴀᴄᴋ', callback_data='help', color=C.BLUE),
            Btn('♻️', callback_data='rfrsh', color=C.GREEN),
            Btn('ʀᴇᴘᴏ', url='https://github.com/mn-bots/ShobanaFilterBot')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        ...
        if settings is not None:
            buttons = [
                [
                    Btn('Filter Button',
                                         callback_data=f'setgs#button#{settings["button"]}#{str(grp_id)}', color=C.BLUE),
                    Btn('Single' if settings["button"] else 'Double',
                                         callback_data=f'setgs#button#{settings["button"]}#{str(grp_id)}')
                ],
                [
                    Btn('Bot PM', callback_data=f'setgs#botpm#{settings["botpm"]}#{str(grp_id)}', color=C.BLUE),
                    Btn('✅ Yes' if settings["botpm"] else '❌ No',
                                         callback_data=f'setgs#botpm#{settings["botpm"]}#{str(grp_id)}',
                                         color=C.GREEN if settings["botpm"] else C.RED)
                ],
                [
                    Btn('File Secure',
                                         callback_data=f'setgs#file_secure#{settings["file_secure"]}#{str(grp_id)}', color=C.BLUE),
                    Btn('✅ Yes' if settings["file_secure"] else '❌ No',
                                         callback_data=f'setgs#file_secure#{settings["file_secure"]}#{str(grp_id)}',
                                         color=C.GREEN if settings["file_secure"] else C.RED)
                ],
                [
                    Btn('IMDB', callback_data=f'setgs#imdb#{settings["imdb"]}#{str(grp_id)}', color=C.BLUE),
                    Btn('✅ Yes' if settings["imdb"] else '❌ No',
                                         callback_data=f'setgs#imdb#{settings["imdb"]}#{str(grp_id)}',
                                         color=C.GREEN if settings["imdb"] else C.RED)
                ],
                [
                    Btn('Spell Check',
                                         callback_data=f'setgs#spell_check#{settings["spell_check"]}#{str(grp_id)}', color=C.BLUE),
                    Btn('✅ Yes' if settings["spell_check"] else '❌ No',
                                         callback_data=f'setgs#spell_check#{settings["spell_check"]}#{str(grp_id)}',
                                         color=C.GREEN if settings["spell_check"] else C.RED)
                ],
                [
                    Btn('Welcome', callback_data=f'setgs#welcome#{settings["welcome"]}#{str(grp_id)}', color=C.BLUE),
                    Btn('✅ Yes' if settings["welcome"] else '❌ No',
                                         callback_data=f'setgs#welcome#{settings["welcome"]}#{str(grp_id)}',
                                         color=C.GREEN if settings["welcome"] else C.RED)
                ]
            ]
            reply_markup = InlineKeyboardMarkup(buttons)
...
async def advantage_spell_chok(client, msg):
    ...
    try:
        ...
        button = [[
                 Btn('ENG', callback_data='esp', color=C.GREEN),
                 Btn('MAL', callback_data='msp', color=C.GREEN),
                 Btn('HIN', callback_data='hsp', color=C.GREEN),
                 Btn('TAM', callback_data='tsp', color=C.GREEN)
        ],[
                 Btn('🔍 ɢᴏᴏɢʟᴇ 🔎', url=f"https://www.google.com/search?q={reqst_gle}", color=C.BLUE)
             ]]
    ...
    btn.append([Btn(text="✘ ᴄʟᴏsᴇ ✘", callback_data=f'spol#{reqstr1}#close_spellcheck', color=C.RED)])
