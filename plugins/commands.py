...
async def create_file_buttons(client, sent_message):
    ...
    try:
        ...
        if invite_link:
            buttons.append([Btn("📢 Join Channel", url=invite_link, color=C.BLUE)])
        buttons.append([Btn("🔗 View File", url=message_link, color=C.BLUE)])
...
async def _attach_stream_buttons(client, sent_msg, file_id: str):
    ...
    try:
        ...
        markup = InlineKeyboardMarkup([[
            Btn("🌐 Stream",   url=links["stream_link"],  color=C.GREEN),
            Btn("⬇ Download", url=links["online_link"],  color=C.GREEN),
        ]])
...
async def checksub_callback(client, callback_query):
    ...
    if await is_subscribed(user_id, client):
        ...
        btn = [[Btn("🤖 Join Updates Channel", url=url, color=C.BLUE)] for url in links.values()]
        btn.append([Btn("🔄 Try Again", callback_data=data, color=C.GREEN)])
    ...
    if not await is_subscribed(message.from_user.id, client):
        ...
        if len(message.command) == 2:
            try:
                ...
                btn.append([Btn("🔄 Try Again", callback_data=f"{pre}#{file_id}", color=C.GREEN)])
            except (IndexError, ValueError):
                btn.append([Btn("🔄 Try Again", url=f"https://t.me/{temp.U_NAME}?start={message.command[1]}", color=C.GREEN)])
    ...
    await message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    Btn(
                        text="YES", callback_data="autofilter_delete", color=C.GREEN
                    )
                ],
                [
                    Btn(
                        text="CANCEL", callback_data="close_data", color=C.RED
                    )
                ],
            ]
        ),
