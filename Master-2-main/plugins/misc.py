...
async def imdb_search(client, message):
    ...
    if data.get("next_page"):
        nav.append(Btn("Next ▶️", callback_data=f"imdbpg#{title}#{data['next_page']}", color=C.GREEN))
    if nav:
        btn.append(nav)
...
async def imdb_page_callback(bot: Client, quer_y: CallbackQuery):
    ...
    nav = []
    if page > 1:
        nav.append(Btn("◀️ Previous", callback_data=f"imdbpg#{title}#{page-1}", color=C.GREEN))
    if data.get("next_page"):
        nav.append(Btn("Next ▶️", callback_data=f"imdbpg#{title}#{data['next_page']}", color=C.GREEN))
    if nav:
        btn.append(nav)
