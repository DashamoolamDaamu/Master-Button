...
CANCEL_MARKUP = InlineKeyboardMarkup(
    [[Btn('Cancel', callback_data='index_cancel', color=C.RED)]]
)
...
async def send_for_index(bot, message):
    ...
    if message.from_user.id in ADMINS:
        buttons = [
            [Btn('Yes', callback_data=f'index#accept#{chat_id}#{last_msg_id}#{message.from_user.id}', color=C.GREEN)],
    ...
    buttons = [
        [Btn('Accept Index', callback_data=f'index#accept#{chat_id}#{last_msg_id}#{message.from_user.id}', color=C.GREEN)],
        [Btn('Reject Index', callback_data=f'index#reject#{chat_id}#{message.id}#{message.from_user.id}', color=C.RED)],
    ]
    await bot.send_message(
        LOG_CHANNEL,
