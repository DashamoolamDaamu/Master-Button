...
def _build_picker_keyboard(
    ...
    if page > 0:
        nav_row.append(Btn("⬅️ Prev", callback_data=f"gdl_page:{user_id}:{page - 1}", color=C.GREEN))
    if page < total_pages - 1:
        nav_row.append(Btn("Next ➡️", callback_data=f"gdl_page:{user_id}:{page + 1}", color=C.GREEN))
    if nav_row:
        buttons.append(nav_row)

    buttons.append([Btn("❌ Cancel", callback_data=f"gdl_cancel:{user_id}", color=C.RED)])
...
async def getdlink_pick_callback(bot: Client, query) -> None:
    ...
    season_row_1 = [Btn("🎬 Movie", callback_data=f"gdl_season:{user_id}:0", color=C.GREEN)]
    season_rows  = [season_row_1]
    row: list[InlineKeyboardButton] = []
    for s in range(1, 11):
        row.append(Btn(f"S{s:02d}", callback_data=f"gdl_season:{user_id}:{s}", color=C.GREEN))
        if len(row) == 5:
            season_rows.append(row)
            row = []
    if row:
        season_rows.append(row)
    season_rows.append([Btn("❌ Cancel", callback_data=f"gdl_cancel:{user_id}", color=C.RED)])
...
async def getdlink_season_callback(bot: Client, query) -> None:
    ...
    confirm_markup = InlineKeyboardMarkup(
        search_btn_markup.inline_keyboard + [[
            Btn("✅ Send to Channels", callback_data=f"gdl_confirm:{user_id}", color=C.GREEN),
            Btn("❌ Cancel",           callback_data=f"gdl_cancel:{user_id}",  color=C.RED),
        ]]
    )
...
def _build_summary_page(
    ...
    if page > 0:
        nav.append(Btn("⬅️ Prev", callback_data=f"sumpage:{page - 1}", color=C.GREEN))
    if page < total_pages - 1:
        nav.append(Btn("Next ➡️", callback_data=f"sumpage:{page + 1}", color=C.GREEN))
...
async def sendupnow_cmd(bot: Client, message) -> None:
    ...
    markup = InlineKeyboardMarkup([[
        Btn("✅ Send Now", callback_data="sendupnow:confirm", color=C.GREEN),
        Btn("❌ Cancel",   callback_data="sendupnow:cancel",  color=C.RED),
    ]])
